...
def test_accetta_ordine():
    piva = get_random_piva()
    ordine_id = 2 # Simuliamo l'ordine con ID 1
    corriere_id = 2  # Simuliamo il corriere con ID 1
...
def test_crea_trasportatore():
    ...
    response = requests.put(f"{BASE_URL}/autentica/", json={"nome": "AmaZZon", "piva": "12345678912", "via" : "Via del Denaro", "civico": 7, "cap" : "77777", "city" : "Napoli", "stato" : "Montecarlo"})
    print(f"Risposta: {response.text}")
