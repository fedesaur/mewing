...
bool cercaProdottiDisponibili(int clientSocket)
{
    ...
    Prodotto *prodottiDisponibili = recuperaProdottiDisponibili(res, clientSocket);
...
*Prodotto recuperaProdottiDisponibili(PGresult *res, int clientSocket)
{

    int rows;
    char comando[1000];
    sprintf(comando, "SELECT pr.id, pr.descrizione, pr.nome, pr.prezzo, fr.nome 
    ...
    if (rows > 0)
    {
        ...
        for (int i = 0; i < rows; i++)
        {
            ...
            std::string prodotto = "ID Prodotto: " + ID + 
            " Nome Prodotto: " + nome +
